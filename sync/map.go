...
// 在两种情况下推荐使用sync.map, 即并发的读写和只写一次多次进行读
// The zero Map is empty and ready for use. A Map must not be copied after first use.
type Map struct {
	mu Mutex

	// read contains(包含) the portion(一部分) of the map's contents(内容) that are safe for concurrent access
	...
	// but updating a previously-expunged entry requires that the entry be copied to the dirty map and unexpunged with mu held.
	read atomic.Value // readOnly， 这部分内容是并发安全的

	// dirty contains the portion of the map's contents that require mu to be held.
	// 对这部分内容操作是需要加锁操作
	...
	// 删除了的数据不会在dirty map中
	// An expunged entry in the clean map must be unexpunged and added to the dirty map before a new value can be stored to it.
	//
	// If the dirty map is nil, the next write to the map will initialize it by making a shallow copy of the clean map, omitting stale entries.
	// 如果这个dirty map是空的，在写入的时候会通过一个干净的map通过浅拷贝来初始化
	dirty map[interface{}]*entry

	// misses counts the number of loads since the read map was last updated that needed to lock mu to determine whether the key was present.
	// 这个参数是记录当前的read map中最后一次更新的记录，当dirty map到一个阈值时会将dirty map的内容转到read map中
	// Once enough misses have occurred to cover the cost of copying the dirty map,
	// the dirty map will be promoted to the read map (in the unamended state) and the next store to the map will make a new dirty copy.
	misses int
}
...
func (m *Map) Load(key interface{}) (value interface{}, ok bool) {
	// 这个方法的设计和机构体中的说明和结构是对应的，即如果在read中找到了，就不需要去加锁了

	read, _ := m.read.Load().(readOnly)
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
		// 判断dirty中的数据已经进入了read中，判断临界情况
...
// 这个方法如果key在map中存在就返回map中的值，并且loaded返回true，不存在则返回value的值，并且loaded返回false
//
func (m *Map) LoadOrStore(key, value interface{}) (actual interface{}, loaded bool) {
	...
	read, _ = m.read.Load().(readOnly)
	if e, ok := read.m[key]; ok {
		// 如果这个key之前被删掉了，将将这个key的数据添加到dirty中
		if e.unexpungeLocked() {
			m.dirty[key] = e
		}
...
func (e *entry) tryLoadOrStore(value interface{}) (actual interface{}, loaded, ok bool) {
	p := atomic.LoadPointer(&e.p)
	if p == expunged {
	...
	// Copy the interface after the first load to make this method more amenable to escape analysis:
	// if we hit the "load" path or the entry is expunged, we shouldn't bother heap-allocating.
	// 这里区value的值是为了
	ic := value
	for {
		// 比较并且交换指针
