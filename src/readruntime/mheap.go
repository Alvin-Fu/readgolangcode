...
// minPhysPageSize is a lower-bound on the physical page size.
// The true physical page size may be larger than this.
// In contrast, sys.PhysPageSize is an upper-bound on the physical page size.
// 最小的物理页的大小
...
type mheap struct {
	...
	// Each mspan appears exactly once.
	//
	...
	allspans []*mspan // all spans out there,保存所有的mspan的指针
...
type mspan struct {
	...
	// 手动回收的列表，如果被设置以后会对gc进行隐藏，这个东西是由谁进行设置
	...
	// If n >= freeindex and allocBits[n/8] & (1<<(n%8)) is 0 then object n is free; otherwise, object n is allocated.
	...
	// The next arena is populated(填充) as the spans request
	// memory to hold gcmarkBits for the next GC cycle as well as allocBits for newly allocated spans.
	//
	...
	// if sweepgen == h->sweepgen - 1, the span is currently being swept 正在清理
	// if sweepgen == h->sweepgen, the span is swept and ready to use	 已经清理可以使用
	// h->sweepgen is incremented by 2 after every GC，用完后加2复原

	sweepgen    uint32
	divMul      uint16     // for divide by elemsize - divMagic.mul
	baseMask    uint16     // if non-0, elemsize is a power of 2, & this will get object allocation base，分配基数
	allocCount  uint16     // number of allocated objects  已经分配的块数
	spanclass   spanClass  // size class and noscan (uint8)
	incache     bool       // being used by an mcache  是否被使用
	state       mSpanState // mspaninuse etc
	needzero    uint8      // needs to be zeroed before allocation
	divShift    uint8      // for divide by elemsize - divMagic.shift
	divShift2   uint8      // for divide by elemsize - divMagic.shift2
	elemsize    uintptr    // computed from sizeclass or from npages  class表中块的大小
...
//
// This only happens the first time a span is allocated(分配) from
// mheap.spanalloc (it is not called when a span is reused).
//
// Write barriers are disallowed here because it can be called from gcWork when allocating new workbufs.
// However, because it's an indirect call from the fixalloc initializer, the compiler can't see this.
//添加新的span，只有这个span在第一次的时候才会被调用， 这是不能写屏障
...
// A spanClass represents(表示) the size class and noscan-ness of a span.
//
// Each size class has a noscan(无需扫描) spanClass and a scan spanClass. The
...
// arenaIndex returns the index into mheap_.arenas of the arena containing metadata for p.
// This index combines of an index into the L1 map and an index into the L2 map and should be used as mheap_.arenas[ai.l1()][ai.l2()].
//
// If p is outside the range of valid heap addresses, either l1() or l2() will be out of bounds.
//	入p超过了有效堆的范围，l1和l2都将超出边界
