...
//     M must have an associated P to execute(执行) Go code, however it can be
//     blocked(阻塞) or in a syscall w/o an associated P.
//
// Design doc at https://golang.org/s/go11sched.

// Worker thread parking/unparking.
// 解释为什么需要平衡工作线程和硬件资源之间的关系和描述这个问题为什么困难
// We need to balance between keeping enough running worker threads to utilize
// available hardware(资源) parallelism and parking excessive running worker threads
...
// 1. Centralize(集中) all scheduler state (would inhibit scalability).
// 2. Direct goroutine handoff(传递). That is, when we ready a new goroutine and there
//    is a spare(多余的) P, unpark(启动) a thread and handoff it the thread and the goroutine.
//    This would lead to thread state thrashing(抖动，可能是不稳定的意思), as the thread that readied the
//    goroutine can be out of work the very next moment, we will need to park it.
//    Also, it would destroy locality of computation as we want to preserve
//    dependent goroutines on the same thread; and introduce(引入) additional latency.
// 3. Unpark an additional thread whenever we ready a goroutine and there is an
//    idle P, but don't do handoff. This would lead to excessive thread parking/
//    unparking as the additional threads will instantly park without discovering
//    any work to do.
//
// The current approach: 介绍当前的方法
...
// Puts the current goroutine into a waiting state（状态） and unlocks the lock.
...
//go:nosplit 判断goroutine是否可以进行自旋
func sync_runtime_canSpin(i int) bool {
	...
	//这个判断表示必须在多核，由空闲的P，并且自旋次数小于4（active_spin）的时候才可以进行自旋
